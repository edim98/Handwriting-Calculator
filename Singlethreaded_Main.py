...
img = Image.open(os.path.abspath("./ProjectImages/image.jpg")).resize((640, 280), Image.ANTIALIAS)
image = ImageTk.PhotoImage(img)
canvas.create_image(0, 0, image=image, anchor="nw")

# Configure the text position
canvas.create_polygon(points, fill="#dddddd", smooth=True)
formula_container = canvas.create_text(320, 30, fill="white", text="Waiting for formula...", font = ("Times New Roman", 15))
result_container = canvas.create_text(320, 95, fill="black", text="This is the result", font=("Times New Roman", 25))

img2 = Image.open(os.path.abspath("./ProjectImages/image.jpg")).resize((300, 110), Image.ANTIALIAS)
image2 = ImageTk.PhotoImage(img2)
image_container = canvas.create_image(170, 140, image=image2, anchor="nw")
image2_array = None
...
for frame in camera.capture_continuous(rawCapture, format='bgr', use_video_port=True):
	...
	# Else just configure the panel to display the current frame
		...
		panel.configure(image=newImage)
...
# Convert the frame into a numpy array for further processing.
	...
	if formula != "":
		if Calc.check_formula_correct(formula):
			...
			formula = "Incorrect Formula, waiting for new formula"
	else:
		formula = "Waiting for formula"
	...
	if (formula != ''):
		print("I got a formula: " + formula)
		canvas.itemconfigure(formula_container, text=formula)
		formula = ''

	# If the result was updated, display it
	if (result != ''):
		print("I got a result: " + result)
		canvas.itemconfigure(result_container, text=result)
		result = ''

	if image2_array is not None:
		np_array = np.array(image2_array)
		...
		canvas.itemconfigure(image_container, image=boxImage)
		image2_array = None
	# Update the GUI
	root.update_idletasks()
	root.update()