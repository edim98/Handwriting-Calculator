        ...
        while self.exit == 0:
            ...
            for frame in camera.capture_continuous(rawCapture, format='bgr', use_video_port=True):
                ...
                if self.image2_array is not None:
                    ...
                    resized = cv2.resize(np_array, None, fx = 1, fy = 0.8, interpolation = cv2.INTER_CUBIC)
                    boxImage = Image.fromarray(resized)
                    #boxImage = Image.fromarray(self.image2_array)
                    boxImage = ImageTk.PhotoImage(boxImage)
                    self.canvas2.itemconfigure(self.image_container, image=boxImage)
                    self.image2_array = None
                # Update the GUI
                self.root.update_idletasks()
                self.root.update()
            ...
            if formula != "":
                if Calc.check_formula_correct(formula):
                    ...
                    try:
                        ...
                        self.pause_flag += 1
                        print('flag: ' + str(self.pause_flag))
                    except:
